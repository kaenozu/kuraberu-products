...
if ($SiteUrl.Scheme -ne 'https' -or $SiteUrl.AbsolutePath -ne '/' -or $SiteUrl.Query -or $SiteUrl.Fragment) {
    throw 'SiteUrl must be an HTTPS site root without query or fragment.'
}
...
try {
    ...
    @(
        ...
        '- Result: **PASS**',
        "- Mode: $($report.mode)",
        "- Repository HEAD / build SHA: $head",
