...
### 山崎実業 神札スタンド リン vs リン シングル（今回実装）

- 記事slug: `yamazaki-ofuda-stand-rin-vs-single`
- 公式個別ページ・仕様・画像を確認済み。楽天は検索結果へ飛ぶ短縮URLを使わず、HTTP 200と型番一致を確認した商品詳細ページへ接続。
- 購入導線: `roomy/ymz22nov22h01`（6141/6142） / `roomy/ymz22nov22h02`（6144）。通常リンクのため広告表記・`sponsored` は付けない。
