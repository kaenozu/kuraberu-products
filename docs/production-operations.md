...
**Dispatch-only trigger.** Production deploys run exclusively via explicit `workflow_dispatch` (`expected_sha` + `confirm: DEPLOY`). Merging to `main` never deploys by itself. Previously a `push` trigger auto-deployed each merge after a CI green-gate poll; on 2026-09-03 that path fired three attempts (#611/#612/#613) against a red main. Removing the push trigger makes that incident class structurally impossible: every production run now has an explicit operator signer, and the SHA guards still reject stale SHAs.
...
3. For machine-readable evidence, re-run the verification against the deployed site with a local output root:
   ...
   **Auto-close policy (optional, default off).** By default every evidence issue stays OPEN for human sign-off (the #361 contract). The repo owner can flip the repository variable `AUTO_CLOSE_PASS` to `true` (`gh variable set AUTO_CLOSE_PASS --body true`, or Settings → Secrets and variables → Actions → Variables): issues whose `report.json` is a **fully-passed verification** (`result=PASS` and every check `PASS`, deployed-SHA check included) are then closed automatically right after creation, with an explanatory comment (result / attempts / checks counts and a Reopen hint). `BLOCKER` and missing-`report.json` issues **always stay OPEN** for human review regardless of the variable. Reopen a closed issue if you disagree with the automated verdict. Clear the variable (or set it to anything other than `true`) to restore manual sign-off for all issues. To create or inspect one manually, either use the issue template or run the script with `--dry-run` against a local report.json:
