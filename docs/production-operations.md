...
> 履歴メモ (#704): かつて `push: main` でも自動デプロイが試行され、2026-09-03 に red な main に対して3件の失敗試行 (#611/#612/#613) が発生したため、push トリガは廃止した。CI green gate（push 時の verify ポーリング）も同時に削除し、手動 dispatch（exact SHA + `confirm: DEPLOY` + SHA ガード）を唯一の正規トリガとした。
