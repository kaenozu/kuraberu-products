...
describe("generic comparison components", () => {
  ...
  it("keeps product-specific copy outside generic components", () => {
    ...
    const visualDiffSource = fs.readFileSync(
      "src/components/VisualKeyDifferences.astro",
      "utf8",
    );

    for (const source of [differenceSource, summarySource, visualDiffSource]) {
      expect(source).not.toContain("パンパース");
      expect(source).not.toContain("肌へのいちばん");
      expect(source).not.toContain("さらさらケア");
      expect(source).not.toContain("メリーズ");
      expect(source).not.toContain("容量0.5L");
      expect(source).not.toContain("保温効力");
      expect(source).not.toContain("サーモス");
      expect(source).not.toContain("タイガー");
    }
